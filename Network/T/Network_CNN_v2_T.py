...
#train_step = tf.train.AdamOptimizer(1e-4).minimize(loss_MSE) 
train_step_displacement = tf.train.AdagradOptimizer(0.002).minimize(loss_MSE_displacement)


def evaluate():
    #saver = tf.train.Saver()
    '''
    #saver.restore(sess, "/summer_research/T/ckpt/Network_CNN_v2.4.2_T_str_model.ckpt")
    total_loss_stress = 0
    ...
    #saver.restore(sess, "/summer_research/T/ckpt/Network_CNN_v2.4.2_T_dis_model.ckpt")
...
def accuracy():
    '''
    saver = tf.train.Saver()
    saver.restore(sess,"/tmp/Network_CNN_v2.4.2_T_str_model.ckpt")
    a = 0
    for k in range(0,test_size):
    ...
    '''
    saver = tf.train.Saver()
    saver.restore(sess, "/summer_research/T/ckpt/Network_CNN_v2.4.2_T_dis_temp_model.ckpt")
    acc = 0
    #acc_list = []
...
def train():
    ...
    for k in range(0,30):
        ...
        saver.save(sess,"/summer_research/T/ckpt/Network_CNN_v2.4.2_T_dis_temp_model.ckpt")    
        #saver.save(sess,"/summer_research/T/ckpt/Network_CNN_v2.4.2_T_str_model.ckpt")
