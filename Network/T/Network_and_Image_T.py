...
N_ = 615   # Number of nodes need output
M =  400  # Number of train cases
test_size = M # Number of test cases
batch_size = 1

file_object = open('T_adjacency.txt')
...
def run_with_input(x,y,z,rst_path):
    ...
    saver.restore(sess, "/summer_research/T/ckpt/Network_CNN_v2.4.2_T_dis_model.ckpt") # displacement
    ...
    label_pngpath = 'D:\\summer_research\\T\\png\\displacement\\original_displacement'+str(x)+'_'+str(y)+'_'+str(z)+'.png'
    #result.plot_nodal_solution(rnum = 0,screenshot=pngpath,interactive=False)
    result.plot_nodal_solution(rnum=0,screenshot=label_pngpath,interactive=False,rng=plot_range)

    
    predict_pngpath = 'D:\\summer_research\\T\\png\\displacement\\prediction_displacement'+str(x)+'_'+str(y)+'_'+str(z)+'.png'
    ...
    tmp = Image.new('RGBA', image.size, TINT_COLOR+(0,))
    draw = ImageDraw.Draw(tmp)
    draw.line([(763,430),(763+x_,430+y_)],fill=TINT_COLOR+(127,),width=10)
    draw.polygon([(763+x_*1.2,430+y_*1.2),(763+x_+y_*0.2,430+y_-x_*0.2),(763+x_-y_*0.2,430+y_+x_*0.2)],fill=TINT_COLOR+(127,))
    #draw.polygon(((850,390),(830,497),(393,250),(383,158)), fill=TINT_COLOR+(127,))#topleft bottomleft bottomright topright
    image = Image.alpha_composite(image, tmp)
    image = image.convert("RGB")
    circle='D:\\summer_research\\T\\png\\displacement\\prediction_circle_displacements'+str(x)+'_'+str(y)+'_'+str(z)+'.png'
    image.save(circle)
    ...
    combined_png.paste(label_png,(1024,0))
    filename='D:\\summer_research\\T\\png\\displacement\\combine_displacement'+str(x)+'_'+str(y)+'_'+str(z)+'.png'
    combined_png.save(filename)
...
for i in range(N):
    ...
    #print(ip)
    #print(ip[6][:-4])
    run_with_input(float(ip[2]),float(ip[3]),float(ip[4][:-4]),path_list[i])
