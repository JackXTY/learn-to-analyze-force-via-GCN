...
inputdata=open('D:\\summer_research\\T\\T_adjacency.txt','w')
j=0
...
for file in os.listdir(path):
    ...
    for n in range(0,N):
        ...
        for i in range(0,N):
            ...
            if (temp_xd<xd or xd==0) and temp_xd>xd*0.1:
                xd = temp_xd
            if (temp_yd<yd or yd==0) and temp_yd>yd*0.1:
                yd = temp_yd
            if (temp_zd<zd or zd==0) and temp_zd>zd*0.1:
                zd = temp_zd   
        ...
        for i in range(0,N):
            ...
            if  (  ( _xd<xd*1.1 and _xd>xd*0.01 and _yd<yd*0.2 and _zd<zd*0.2)
                or ( _yd<yd*1.1 and _yd>yd*0.01 and _xd<xd*0.2 and _zd<zd*0.2)
                or ( _zd<zd*1.1 and _zd>zd*0.01 and _xd<xd*0.2 and _yd<yd*0.2) ):
                result[n][i] = 1
                result[i][n] = 1
